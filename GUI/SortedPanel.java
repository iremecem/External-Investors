	...
	public SortedPanel(Stock stock, JFrame topFrame ) {
		...
		addMouseListener(new MouseAdapter() {
			public void mouseExited(MouseEvent e) {
				...
				System.out.println(topFrame == null);
				System.out.println("BÝZ BURDAYIZ Mouser Cursor");
				setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
			}
		});
		
		add (companyName);
//		add (new JLabel("              " + String.format("%.4f", stock.getBuyValue())));
//		add (new JLabel("                     " + String.format("%.4f", stock.getSellValue())));
		...
		//JLabel lblChange = new JLabel(" "); //+ stock.getDifference(startTime, endTime) );
